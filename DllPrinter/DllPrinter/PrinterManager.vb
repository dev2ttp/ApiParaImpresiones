...
Public Class PrinterManager
    ...
    Public Function ImprimirPdf(docByte64Array() As Byte, sendToDefaultPrinter As Boolean, nombreImpresora As String, tipoImpresora As String) As PrintResponseModel
        Try
            ...
            Dim filename As String = String.Format("Doc_{0}_{1}.pdf", DateTime.Now.ToString("yyyyMMdd"), DateTime.Now.ToString("HHmm"))
            Dim filePath As String = Path.Combine(sPath, filename)

            Using stream As New FileStream(filePath, FileMode.Create), writer As New BinaryWriter(stream)
                Dim documentoArray() As Byte = docByte64Array
                writer.Write(documentoArray, 0, documentoArray.Length)
                writer.Close()
                stream.Close()
    ...
    Private Sub SendToPrinter(fileName As String, sendToDefaultPrinter As Boolean, nombreImpresora As String, tipoImpresora As String)
        ...
        info.Verb = "print"
        If Not sendToDefaultPrinter Then
            info.Verb = "PrintTo"
            info.Arguments = """" + nombreImpresora + """"
        End If
    ...
    Private Sub KillAdobe()
        For Each clsProcess As Process In Process.GetProcesses().Where(Function(x) x.ProcessName.StartsWith("AcroRd32"))
            clsProcess.Kill()
        Next
    End Sub
